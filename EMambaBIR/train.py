...
def train(datadir,
    ...
    random.shuffle(train_vol_names)  # 随机打乱文件顺序
    ...
    # prepare the model
    model = vxm.EMambaBIR.EMambaBIRnet(inshape)
    model.to(device)
    if load_model != False:
    ...
    for epoch in range(initial_epoch, epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            labels = [torch.from_numpy(d).to(device).float().permute(0, 4, 1, 2, 3) for d in labels]  #
            source = inputs[0]
            target = inputs[1]
            ...
            for i, Loss in enumerate(Losses):
                curr_loss = Loss(pred[i], labels[i]) * Weights[i]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model-dir', default='model_new_mamba', help='model output directory (default: models)')
