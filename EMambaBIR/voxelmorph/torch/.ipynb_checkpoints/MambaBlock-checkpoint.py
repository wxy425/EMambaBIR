...
def mambatran(x):
    ...
    up = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    x = x.to(device)
    transformer = transformer.to(device)
    embed_dim = 96
    reg_head_chan = 16
    up1 = DecoderBlock(embed_dim * 8, embed_dim * 4, skip_channels=embed_dim * 4,
                       use_batchnorm=False)  # 384, 20, 20, 64
    up2 = DecoderBlock(embed_dim * 4, embed_dim*2, skip_channels=embed_dim*2,
                       use_batchnorm=False)  # 384, 40, 40, 64
    up3 = DecoderBlock(embed_dim*2, embed_dim, skip_channels=embed_dim,
                       use_batchnorm=False)  # 384, 80, 80, 128
    up4 = DecoderBlock(embed_dim , reg_head_chan,
                       skip_channels=reg_head_chan,
                       use_batchnorm=False)  # 384
    c1 = Conv3dReLU(1, embed_dim // 2, 3, 1, use_batchnorm=True)
    c2 = Conv3dReLU(1, reg_head_chan, 3, 1, use_batchnorm=True)
